...
#set up variables
IRfolder=collection
artifacts_dir=${IRfolder}/artifacts
...
syslog -T UTC > ${artifacts_dir}/appleSystemLogs.txt
...
do
	...
	mkdir ${artifacts_dir}/"$dirname"
	ditto "$x" ${artifacts_dir}/"$dirname"
done

#collect privileged files
for x in "${files[@]}"
do
	ditto "$x"* ${artifacts_dir}
done
...
do
	...
	do
		...
		if [ -f $fileLocation ]; then
			ditto "$fileLocation"* ${artifacts_dir}
		fi
	done
done
